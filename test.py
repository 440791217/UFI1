import sys
import os
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QFileDialog, QGroupBox, QComboBox, QMessageBox
import subprocess
import json
...
class Analyzer:
    ...
    def show_directory_selection_panel(self):
        ...
        self.mode_combobox.addItems(["RF", "INST", "MEM"])
        ...
        cancel_button.clicked.connect(self.panel.close)
        button_layout.addWidget(ok_button)
        button_layout.addWidget(cancel_button)
        layout.addLayout(button_layout)

        self.panel.setLayout(layout)
        self.panel.setWindowTitle('目录选择面板')
        self.panel.show()

    def select_directory(self):
        directory = QFileDialog.getExistingDirectory(self.root, '选择目录', directory=os.path.join(self.rootpath, 'out'))
        if directory:
    ...
    def ok_button_clicked(self):
        ...
        try:
            result = subprocess.run([sys.executable, self.analyze_script_path, selected_path], capture_output=True, text=True)
            if result.returncode == 0:
                try:
                    json_result = json.loads(result.stdout)
                    # 这里可以根据实际需求处理 JSON 数据
                    print("JSON 结果:", json_result)
                    QMessageBox.information(self.root, "提示", f"数据处理成功。\nJSON 结果：{json_result}")
                except json.JSONDecodeError:
                    QMessageBox.warning(self.root, "提示", "无法解析脚本返回的 JSON 数据。")
            else:
                QMessageBox.warning(self.root, "提示", f"数据处理失败，错误信息：{result.stderr}")
